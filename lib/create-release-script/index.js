...
const generateReleaseData = (core) => {
  ...
  const releaseJsonContent = core.getInput('releaseJsonContent')
...
const createReleaseScript = async ({ version, components, deployment, deployFilePath, deployTemplateFilePath, releaseJsonPath, releasePath, releaseJsonContent }) => {

  const deployHeader = generateHeader(version, components, deployment, releaseJsonPath, releasePath, releaseJsonContent)
...
const generateHeader = (version, components, deployment, releaseJsonPath, releasePath, releaseJsonContent) => {
  var headerLines = []
  // const dateTimeStamp = format(new Date(),'yyyy-MM-dd:HH:mm:SS')
  const dateTimeStamp = new Date().toJSON().slice(0, 19).replace('T', ':')
  ...
  headerLines.push(`  RELEASE_JSON_CONTENT: '${releaseJsonContent}'\n`)
  return headerLines
}
