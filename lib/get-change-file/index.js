...
const run = async () => {
  try {
    ...
    const changeFileData = changeFileData.data.files.find(isNewChangeFile)

    // if (changeFiles.length > 1) core.setFailed('Too many change files discovered. Please merge the changes files into one.')
    if (!changeFileData) core.setFailed('A change file was not detected. Please create one.')
    else {

      const changeFile = await fs.readFile(changeFileData.filename, { encoding: 'utf8' });
