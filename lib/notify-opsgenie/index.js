...
try {
  ...
  if (jobStatus === 'failure') {
    ...
    closeAlertOnOpsGenie({ alias, runUrl }).catch(error => core.setFailed(error.message))
  }
}
catch (error) {
...
async function closeAlertOnOpsGenie ({ alias, runUrl }) {
  const listPayload = {
  ...
  const closeArertData = {
    ...
    note: `Job ${alias} executed successfully. Details: ${runUrl}`,
  }
