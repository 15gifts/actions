...
const getCancelledRuns2 = async (owner, repo, runId) => {
  // debugging
  console.log('Owner:', owner)
  console.log('Repo:', repo)
  console.log('Run ID:', runId)
  ...
  try {
    const allJobs = octokit.paginate.iterator(octokit.actions.listJobsForWorkflowRun, {
      owner: owner,
      repo: repo,
      run_id: runId,
    })
    ...
    console.log('Cancelled Jobs2:', cancelledJobs) // Debugging
...
async function test () {
  ...
  await getCancelledRuns2(context.repo.owner, context.repo.repo, process.env.GITHUB_RUN_ID)
}
