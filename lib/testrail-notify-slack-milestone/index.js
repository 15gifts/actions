...
const passPercentage = (run) => {
  const total = run.untested_count + run.retest_count + run.failed_count + run.passed_count
...
const constructPayload = (channel, iconEmoji, username, milestone, actionLink, passedRuns, failedRuns, cancelledJobs) => {
  ...
  cancelledJobs.forEach(run => {
    payload.blocks.push({
      ...
      'elements': [{
        'type': 'mrkdwn',
        'text': `${lookupEmoji(extractGithubName(run.name).split(',')[0].trim())}`,
      },
      {
        'type': 'mrkdwn',
...
const run = async () => {
  try {
    ...
    const runs = await api.getRuns(projectId, filters)
    ...
    runs.forEach(run => {
      if (run.untested_count + run.retest_count + run.failed_count === 0) {
        passedRuns.push(run)
      }
      else {
        failedRuns.push(run)
