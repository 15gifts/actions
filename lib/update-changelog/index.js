...
const run = () => {
  try {
    ...
    //const getGitTag = function(callback){
    //  execute("git config --global user.name", function(gitTag){
    //      callback({ gitTag: gitTag.replace("\n", "") });
    //  });
  //};
    ...
    //(err, stdout, stderr) => console.log(stdout));
    //const new_tag = exec('git describe --abbrev=0 --tags "$(git rev-list --tags --skip=1 --max-count=1)"', 
    //function(err, stdout, stderr) {return stdout.trim()} );

    //const { promisify } = require('util');
    //const exec = promisify(require('child_process').exec)
    
    //module.exports.getGitTag = async function getGitTag () {
      // Exec output contains both stderr and stdout outputs
    //  const GitTag = await exec('git describe --abbrev=0 --tags "$(git rev-list --tags --skip=1 --max-count=1)"')
    
    //  return GitTag.stdout.trim();
    //};

    //const new_tag = getGitTag();
