...
const updateChangelog = async ({ dateStamp, changelogFilePath, changes }) => {
...
const generateChanges = (changes) => {
  return Object.entries(changes).forEach(([key, value]) => {
    if (value) {
      newLogs = newLogs + '### ' + `${key}`;
      return newLogs + '- ' + `${value}`;
    }
  });
}
